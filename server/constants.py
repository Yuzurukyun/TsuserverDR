...
class Constants():
    ...
    @staticmethod
    def command_load_asset(client: ClientManager.Client, file: str, file_type: str):
        ...
        if not file:
            ...
            msg = f'the default {name} file'
        else:
            source_file = f'config/{name.replace(" ", "_")}/{file}.yaml'
            msg = f'the custom {name} file `{source_file}`'
